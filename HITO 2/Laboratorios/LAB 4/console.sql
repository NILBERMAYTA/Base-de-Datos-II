...
select au.nombre
from libro as lib inner join autor au on lib.autor_id = au.id
where au.nacionalidad = 'Argentino';


create view Argentinos as
select au.nombre
from libro as lib inner join autor au on lib.autor_id = au.id
where au.nacionalidad = 'Argentino';
...
select li.titulo as titleBook,
       li.editorial as editorialBook,
       li.paginas as pagesBook,
       (
           case
               when li.paginas >1 and li.paginas <=30 then 'CONTENIDO BASICO'
               when li.paginas >30 and li.paginas <=80 then 'CONTENIDO MEDIANO'
                when li.paginas >80 and li.paginas <=150 then 'CONTENIDO SUPERIOR'
           else 'CONTENIDO AVANZADO'
           END
           )
from libro as li;

alter view BookContent as
select li.titulo as titleBook,
       li.editorial as editorialBook,
       li.paginas as pagesBook,
       (
...
select count(*)
from BookContent
where siu='CONTENIDO MEDIANO';

create view bookanduser as
select
    concat(li.titulo,'-',li.editorial,'-',c.nombre) as BookDetail,
    ...
    inner join categoria c on c.id = lc.categoria_id;
